#!/usr/bin/env python3
"""
SOAP-klient til Brugerdatabasen wsiBRUGER v7.
...
ENDPOINT = "https://brugerdatabasen.stil.dk/bpi/wsibruger/7"
...
_NS = {
    ...
    "bru":  "https://brugerdatabasen.stil.dk/bpi/wsibruger/7",
}
...
def hello_world_with_certificate() -> str:
    ...
    body = etree.Element(_q("bru", "helloWorldWithCertificate"))
    return _call(f"{_NS['bru']}/helloWorldWithCertificate", body)


def hent_brugeres_kontaktpersoner(instnr: str, brugerid: str) -> str:
    """Henter kontaktpersoner tilknyttet en elev på en given institution."""
    body = etree.Element(_q("bru", "hentBrugersKontaktpersoner"))
    etree.SubElement(body, _q("bru", "instnr")).text = instnr
    etree.SubElement(body, _q("bru", "brugerid")).text = brugerid
    return _call(f"{_NS['bru']}/hentBrugersKontaktpersoner", body)


def hent_kontaktpersons_brugere(brugerid: str, instnr: str | None = None) -> str:
    """Henter de elever en kontaktperson er tilknyttet. instnr begrænser til én institution."""
    body = etree.Element(_q("bru", "hentKontaktpersonsBrugere"))
    if instnr:
        etree.SubElement(body, _q("bru", "instnr")).text = instnr
    etree.SubElement(body, _q("bru", "brugerid")).text = brugerid
    return _call(f"{_NS['bru']}/hentKontaktpersonsBrugere", body)


def hent_brugeres_institutionstilknytninger(brugerid: str) -> str:
    """Henter alle institutioner og roller som en bruger er tilknyttet."""
    body = etree.Element(_q("bru", "hentBrugersInstitutionstilknytninger"))
    etree.SubElement(body, _q("bru", "brugerid")).text = brugerid
    return _call(f"{_NS['bru']}/hentBrugersInstitutionstilknytninger", body)


def hent_data_aftaler() -> str:
    """Henter liste over dataaftaler for udbyderssystemet."""
    body = etree.Element(_q("bru", "hentDataAftaler"))
    return _call(f"{_NS['bru']}/hentDataAftaler", body)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="wsibruger.py",
        description="STIL wsiBRUGER v7 — kommandolinjeklient",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Tilgængelige funktioner:
  hello                          Test certifikatforbindelsen
  kontakter    instnr --bruger ID  Kontaktpersoner for en elev
  elever      --bruger ID [instnr] Elever tilknyttet en kontaktperson
  tilknytninger --bruger ID        Alle institutioner for en bruger
...
""",
    ...
    parser.add_argument(
        "funktion",
        choices=["hello", "kontakter", "elever", "tilknytninger", "aftaler"],
        metavar="funktion",
    ...
    parser.add_argument(
        ...
        nargs="*",
        metavar="instnr",
        help="Institutionsnummer (til kontakter)",
    )
    parser.add_argument("--bruger",      metavar="ID", help="Bruger-ID")
    parser.add_argument("--institution", metavar="NR", help="Institutionsnummer (valgfrit til elever)")
    parser.add_argument(
        "--output", "-o",
    ...
    if fn == "hello":
        ...
        ok = kald("bru_hello.xml", hello_world_with_certificate)

    elif fn == "kontakter":
        if not insts or not args.bruger:
            parser.error("kontakter kræver instnr og --bruger ID")
        log.info("Henter kontaktpersoner for bruger %s på institution %s",
                 args.bruger, insts[0])
        ok = kald(f"bru_kontakter_{insts[0]}_{args.bruger}.xml",
                  hent_brugeres_kontaktpersoner, insts[0], args.bruger)

    elif fn == "elever":
        if not args.bruger:
            parser.error("elever kræver --bruger ID")
        log.info("Henter elever for kontaktperson %s", args.bruger)
        ok = kald(f"bru_elever_{args.bruger}.xml",
                  hent_kontaktpersons_brugere, args.bruger, args.institution)

    elif fn == "tilknytninger":
        if not args.bruger:
            parser.error("tilknytninger kræver --bruger ID")
        log.info("Henter institutionstilknytninger for bruger %s", args.bruger)
        ok = kald(f"bru_tilknytninger_{args.bruger}.xml",
                  hent_brugeres_institutionstilknytninger, args.bruger)

    elif fn == "aftaler":
        log.info("Henter dataaftaler")
        ok = kald("bru_aftaler.xml", hent_data_aftaler)
