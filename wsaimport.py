#!/usr/bin/env python3
"""
SOAP-klient til Brugerdatabasen wsaIMPORT v8.

WS-Security: RSA-SHA256 med eksklusiv C14N.
Signerede elementer: Body, Timestamp, MessageID, UdbydersystemId.

OBS: Import-operationerne skriver data til STILs Brugerdatabase og bør
kun bruges, når du er sikker på, at input-XML er korrekt formateret.
"""
...
ENDPOINT = "https://brugerdatabasen.stil.dk/bpi/wsaimport/8"
...
_NS = {
    ...
    "imp":  "https://brugerdatabasen.stil.dk/bpi/wsaimport/8",
}
...
def _xml_body(operation: str, xml_fil: Path) -> etree._Element:
    """Indlæser en XML-fil og pakker den ind i et import-body-element."""
    inst_xml_doc = etree.parse(str(xml_fil))
    body = etree.Element(_q("imp", operation))
    inst_xml_el = etree.SubElement(body, _q("imp", "instXML"))
    inst_xml_el.append(inst_xml_doc.getroot())
    return body
...
def hello_world_with_certificate() -> str:
    ...
    body = etree.Element(_q("imp", "helloWorldWithCertificate"))
    return _call(f"{_NS['imp']}/helloWorldWithCertificate", body)


def importer_xml(xml_fil: Path) -> str:
    """Fuld import til SkoleGrunddata. Erstatter alle eksisterende data for institutionen."""
    body = _xml_body("importerXml", xml_fil)
    return _call(f"{_NS['imp']}/importerXml", body)


def importer_delta_xml(xml_fil: Path) -> str:
    """Delta-import — opdaterer kun de brugere der er ændrede siden sidst. Primær importmetode."""
    body = _xml_body("importerDeltaXml", xml_fil)
    return _call(f"{_NS['imp']}/importerDeltaXml", body)


def importer_slet_xml(xml_fil: Path) -> str:
    """Slet-import — fjerner de angivne brugere fra SkoleGrunddata. Bruges sammen med delta-import."""
    body = _xml_body("importerSletXml", xml_fil)
    return _call(f"{_NS['imp']}/importerSletXml", body)


def hent_data_aftaler() -> str:
    """Henter liste over dataaftaler for udbyderssystemet."""
    body = etree.Element(_q("imp", "hentDataAftaler"))
    return _call(f"{_NS['imp']}/hentDataAftaler", body)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="wsaimport.py",
        description="STIL wsaIMPORT v8 — kommandolinjeklient",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Tilgængelige funktioner:
  hello                      Test certifikatforbindelsen
  fuld      --xml FIL        Fuld import (erstatter alle data for institutionen)
  delta     --xml FIL        Delta-import af ændrede brugere (primær importmetode)
  slet      --xml FIL        Slet-import — fjerner angivne brugere
  aftaler                    Dataaftaler for udbyderssystemet

OBS: Import-operationerne skriver til STILs Brugerdatabase. Vær sikker
på, at input-XML er korrekt formateret og kontrolleret, inden du kører.
Se STILs SkoleGrunddata-dokumentation for XML-formatet.
...
""",
    ...
    parser.add_argument(
        "funktion",
        choices=["hello", "fuld", "delta", "slet", "aftaler"],
        metavar="funktion",
    ...
    parser.add_argument(
        "--xml", "-x",
        metavar="FIL",
        help="Sti til XML-fil med importdata (påkrævet til fuld, delta og slet)",
    )
    parser.add_argument(
        "--output", "-o",
        default=str(BASE_DIR),
        metavar="MAPPE",
        help="Mappe til svarfiler (standard: projektmappen)",
    )
    ...
    if fn == "hello":
        ...
        ok = kald("imp_hello.xml", hello_world_with_certificate)

    elif fn in ("fuld", "delta", "slet"):
        if not args.xml:
            parser.error(f"{fn} kræver --xml FIL")
        xml_sti = Path(args.xml)
        if not xml_sti.exists():
            parser.error(f"XML-filen findes ikke: {xml_sti}")

        if fn == "fuld":
            log.info("Starter fuld import fra %s", xml_sti)
            ok = kald("imp_svar_fuld.xml", importer_xml, xml_sti)
        elif fn == "delta":
            log.info("Starter delta-import fra %s", xml_sti)
            ok = kald("imp_svar_delta.xml", importer_delta_xml, xml_sti)
        elif fn == "slet":
            log.info("Starter slet-import fra %s", xml_sti)
            ok = kald("imp_svar_slet.xml", importer_slet_xml, xml_sti)

    elif fn == "aftaler":
        log.info("Henter dataaftaler")
        ok = kald("imp_aftaler.xml", hent_data_aftaler)
