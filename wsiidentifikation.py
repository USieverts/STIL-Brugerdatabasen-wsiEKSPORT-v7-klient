#!/usr/bin/env python3
"""
SOAP-klient til Brugerdatabasen wsiIDENTIFIKATION v6.

WS-Security: RSA-SHA256 med eksklusiv C14N.
Signerede elementer: Body, Timestamp, MessageID, UdbydersystemId.

OBS: Svarfiler indeholder CPR-numre og er gitignored som idi_*.xml.
"""
...
ENDPOINT = "https://brugerdatabasen.stil.dk/bpi/wsiidentifikation/6"
...
_NS = {
    ...
    "idi":  "https://brugerdatabasen.stil.dk/bpi/wsiidentifikation/6",
}
...
def hello_world_with_certificate() -> str:
    ...
    body = etree.Element(_q("idi", "helloWorldWithCertificate"))
    return _call(f"{_NS['idi']}/helloWorldWithCertificate", body)


def hent_brugerid_fra_cpr(cpr: str) -> str:
    """Slår UniLogin-brugerid (UniID) op ud fra et CPR-nummer."""
    body = etree.Element(_q("idi", "hentBrugeridFraCpr"))
    etree.SubElement(body, _q("idi", "cpr")).text = cpr
    return _call(f"{_NS['idi']}/hentBrugeridFraCpr", body)


def hent_cpr_fra_brugerid(brugerid: str) -> str:
    """Slår CPR-nummer op ud fra et UniLogin-brugerid (UniID)."""
    body = etree.Element(_q("idi", "hentCprFraBrugerid"))
    etree.SubElement(body, _q("idi", "brugerid")).text = brugerid
    return _call(f"{_NS['idi']}/hentCprFraBrugerid", body)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="wsiidentifikation.py",
        description="STIL wsiIDENTIFIKATION v6 — kommandolinjeklient",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Tilgængelige funktioner:
  hello                        Test certifikatforbindelsen
  brugerid  --cpr XXXXXXXXXX   Hent UniID for et CPR-nummer
  cpr      --bruger ID         Hent CPR-nummer for et UniLogin-brugerid

OBS: Svarfiler indeholder CPR-numre og gemmes som idi_*.xml (gitignored).
...
""",
    ...
    parser.add_argument(
        "funktion",
        choices=["hello", "brugerid", "cpr"],
        metavar="funktion",
        help="Funktion der skal kaldes (se liste nedenfor)",
    )
    parser.add_argument("--cpr",    metavar="XXXXXXXXXX", help="CPR-nummer (10 cifre, ingen bindestreg)")
    parser.add_argument("--bruger", metavar="ID",         help="UniLogin-brugerid")
    parser.add_argument(
        "--output", "-o",
    ...
    if fn == "hello":
        ...
        ok = kald("idi_hello.xml", hello_world_with_certificate)

    elif fn == "brugerid":
        if not args.cpr:
            parser.error("brugerid kræver --cpr XXXXXXXXXX")
        log.info("Slår UniID op for CPR-nummer")
        ok = kald("idi_brugerid.xml", hent_brugerid_fra_cpr, args.cpr)

    elif fn == "cpr":
        if not args.bruger:
            parser.error("cpr kræver --bruger ID")
        log.info("Slår CPR op for bruger %s", args.bruger)
        ok = kald(f"idi_cpr_{args.bruger}.xml", hent_cpr_fra_brugerid, args.bruger)
